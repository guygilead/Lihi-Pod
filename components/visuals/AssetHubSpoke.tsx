...
function SpokeChip({
  ...
  return (
    <div
      className={cn(
        "flex items-center gap-2 rounded-2xl border bg-paper px-3.5 py-2.5 shadow-soft",
        spoke.tone === "gold"
...
function Hub({ label }: { label: string }) {
  return (
    <div className="flex h-32 w-32 flex-col items-center justify-center gap-1.5 rounded-full bg-gradient-to-b from-charcoal to-charcoal-deep text-center shadow-[var(--shadow-lift)] ring-1 ring-line-dark lg:h-40 lg:w-40 lg:shadow-[0_0_0_8px_rgba(176,133,66,0.1),var(--shadow-lift)]">
      <MicGlyph className="w-6 text-gold-soft lg:w-7" strokeWidth={1.8} />
      <span className="px-3 text-sm font-semibold text-cream">{label}</span>
    </div>
  );
}
...
export function AssetHubSpoke() {
  ...
  const radius = 41; // percent of the square container
    ...
    <>
      ...
      <div className="relative mx-auto hidden aspect-square w-full max-w-2xl lg:block">
        ...
        {spokes.map((s, i) => (
          <div
            key={s.label}
            className="absolute z-10 w-max max-w-[9rem] -translate-x-1/2 -translate-y-1/2"
      ...
      {/* Mobile / tablet: hub on top, chips in a grid */}
